...
@interface CCTreasureBoxResultView ()
...
@implementation CCTreasureBoxResultView

+ (void)showWish:(NSArray<JFLotteryResultItem *> *)models callback:(ClickableCallback)callback
{
    CCTreasureBoxResultView *view = [[NSBundle mainBundle] loadNibNamed:@"CCTreasureBoxResultView" owner:self options:nil][0];
...
- (void)expandItemViewsWithItemViewList:(NSMutableArray<CCTreasureBoxResultItemView *> *)itemViewList
{
    [self layoutIfNeeded];
    
    if (itemViewList.count == 1) {
        CCTreasureBoxResultItemView *itemView = itemViewList[0];
        ...
        for (int i = 0; i < itemViewList.count; i++) {
            CCTreasureBoxResultItemView *itemView = itemViewList[i];
...
- (void)setModels:(NSArray<JFLotteryResultItem *> *)models
{
    ...
    UINib *itemViewNib = [UINib nibWithNibName:@"CCTreasureBoxResultItemView" bundle:nil];
    NSMutableArray<CCTreasureBoxResultItemView *> *itemViewList = [NSMutableArray array];
    ...
    for (JFLotteryResultItem *item in models) {
        ...
        CCTreasureBoxResultItemView *itemView = [itemViewNib instantiateWithOwner:nil options:nil].firstObject;
        itemView.model = item;
