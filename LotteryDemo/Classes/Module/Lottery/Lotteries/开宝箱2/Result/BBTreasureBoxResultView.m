...
#import "BBTreasureBoxResultItemView.h"
#import "JFHttpRequestHelper.h"
#import <AudioToolbox/AudioToolbox.h>

@interface BBTreasureBoxResultView ()
...
@implementation BBTreasureBoxResultView

+ (void)showWish:(NSArray<JFLotteryResultItem *> *)models callback:(ClickableCallback)callback
{
    BBTreasureBoxResultView *view = [[NSBundle mainBundle] loadNibNamed:@"BBTreasureBoxResultView" owner:self options:nil][0];
...
- (void)expandItemViewsWithItemViewList:(NSMutableArray<BBTreasureBoxResultItemView *> *)itemViewList
{
    [self layoutIfNeeded];
    
    if (itemViewList.count == 1) {
        BBTreasureBoxResultItemView *itemView = itemViewList[0];
        ...
        CGFloat itemWidth = (SCREEN_WIDTH - 75) / colCount;
        CGFloat itemHeight = 100;
        
        // 为了让2/3个礼物也居中
        CGFloat offsetX = 0;
        if (itemViewList.count == 2) {
            offsetX = itemWidth * 0.7;
        } else if (itemViewList.count == 3) {
            offsetX = itemWidth * 0.18;
        }
        
        for (int i = 0; i < itemViewList.count; i++) {
            BBTreasureBoxResultItemView *itemView = itemViewList[i];
        ...
        [UIView animateWithDuration:0.28 animations:^{
            for (int i = 0; i < itemViewList.count; i++) {
                ...
                CGFloat margin = ((SCREEN_WIDTH - 75) - (itemWidth * colCount)) / (colCount + 1);
...
- (void)setModels:(NSArray<JFLotteryResultItem *> *)models
{
    ...
    UINib *itemViewNib = [UINib nibWithNibName:@"BBTreasureBoxResultItemView" bundle:nil];
    NSMutableArray<BBTreasureBoxResultItemView *> *itemViewList = [NSMutableArray array];
    ...
    for (JFLotteryResultItem *item in models) {
        ...
        BBTreasureBoxResultItemView *itemView = [itemViewNib instantiateWithOwner:nil options:nil].firstObject;
        itemView.model = item;
    ...
    if (itemViewList.count <= 3) {
        ...
        self.contentView_right.constant = 50;
    } else {
        self.contentView_left.constant = 20;
        self.contentView_right.constant = 20;
    }
