...
@interface FFTurntableResultView ()
...
@implementation FFTurntableResultView

+ (void)showWish:(NSArray<JFLotteryResultItem *> *)models
{
    FFTurntableResultView *view = [[NSBundle mainBundle] loadNibNamed:@"FFTurntableResultView" owner:self options:nil][0];
...
- (void)expandItemViewsWithItemViewList:(NSMutableArray<FFTurntableResultItemView *> *)itemViewList
{
    [self layoutIfNeeded];
    
    if (itemViewList.count == 1) {
        FFTurntableResultItemView *itemView = itemViewList[0];
        ...
        for (int i = 0; i < itemViewList.count; i++) {
            FFTurntableResultItemView *itemView = itemViewList[i];
...
- (void)setModels:(NSArray<JFLotteryResultItem *> *)models
{
    ...
    UINib *itemViewNib = [UINib nibWithNibName:@"FFTurntableResultItemView" bundle:nil];
    NSMutableArray<FFTurntableResultItemView *> *itemViewList = [NSMutableArray array];
    ...
    for (JFLotteryResultItem *item in models) {
        ...
        FFTurntableResultItemView *itemView = [itemViewNib instantiateWithOwner:nil options:nil].firstObject;
        itemView.model = item;
