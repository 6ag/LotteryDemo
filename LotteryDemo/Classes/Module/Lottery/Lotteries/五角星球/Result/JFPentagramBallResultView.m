...
#import "JFPentagramBallResultItemView.h"
#import "JFHttpRequestHelper.h"
#import <AudioToolbox/AudioToolbox.h>

@interface JFPentagramBallResultView ()
...
@implementation JFPentagramBallResultView

+ (void)showWish:(NSArray<JFLotteryResultItem *> *)models
{
    JFPentagramBallResultView *view = [[NSBundle mainBundle]loadNibNamed:@"JFPentagramBallResultView" owner:self options:nil][0];
...
- (void)expandItemViewsWithItemViewList:(NSMutableArray<JFPentagramBallResultItemView *> *)itemViewList
{
    [self layoutIfNeeded];
    ...
    for (int i = 0; i < itemViewList.count; i++) {
        JFPentagramBallResultItemView *itemView = itemViewList[i];
...
- (void)setModels:(NSArray<JFLotteryResultItem *> *)models
{
    ...
    UINib *itemViewNib = [UINib nibWithNibName:@"JFPentagramBallResultItemView" bundle:nil];
    NSMutableArray<JFPentagramBallResultItemView *> *itemViewList = [NSMutableArray array];
    ...
    for (JFLotteryResultItem *item in models) {
        ...
        JFPentagramBallResultItemView *itemView = [itemViewNib instantiateWithOwner:nil options:nil].firstObject;
        itemView.model = item;
