...
#import "JFWaterTreeResultItemView.h"
#import <AudioToolbox/AudioToolbox.h>

@interface JFWaterTreeResultView ()
...
@implementation JFWaterTreeResultView

+ (void)showWish:(NSArray<JFLotteryResultItem *> *)models
{
    JFWaterTreeResultView *view = [[NSBundle mainBundle] loadNibNamed:@"JFWaterTreeResultView" owner:self options:nil][0];
...
- (void)expandItemViewsWithItemViewList:(NSMutableArray<JFWaterTreeResultItemView *> *)itemViewList
{
    [self layoutIfNeeded];
    
    if (itemViewList.count == 1) {
        JFWaterTreeResultItemView *itemView = itemViewList[0];
        ...
        for (int i = 0; i < itemViewList.count; i++) {
            JFWaterTreeResultItemView *itemView = itemViewList[i];
...
- (void)setModels:(NSArray<JFLotteryResultItem *> *)models
{
    ...
    UINib *itemViewNib = [UINib nibWithNibName:@"JFWaterTreeResultItemView" bundle:nil];
    NSMutableArray<JFWaterTreeResultItemView *> *itemViewList = [NSMutableArray array];
    ...
    for (JFLotteryResultItem *item in models) {
        ...
        // 创建礼物item视图
        JFWaterTreeResultItemView *itemView = [itemViewNib instantiateWithOwner:nil options:nil].firstObject;
        itemView.model = item;
